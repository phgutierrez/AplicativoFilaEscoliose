...
app.config['WTF_CSRF_SECRET_KEY'] = os.getenv('WTF_CSRF_SECRET_KEY', 'csrf_key_123')
...
def painel():
    ...
    with get_db_connection() as conn:
        ...
        total_operados = conn.execute("SELECT COUNT(*) FROM operados").fetchone()[0]
...
def realizar_cirurgia(paciente_id):
    ...
    with get_db_connection() as conn:
        try:
            conn.execute("BEGIN TRANSACTION")
            paciente = conn.execute("SELECT nome, nascimento FROM pacientes WHERE id = ?", (paciente_id,)).fetchone()
            ...
            conn.execute("DELETE FROM consultas WHERE paciente_id = ?", (paciente_id,))
            conn.execute("COMMIT")
...
def exportar_operados():
    temp_file = get_temp_file_path(prefix='pacientes_operados_', suffix='.xlsx')
    try:
        with get_db_connection() as conn:
            df = pd.read_sql_query("SELECT * FROM operados ORDER BY data_realizacao DESC", conn)
        df.to_excel(temp_file, index=False)
...
def importar_planilha():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            colunas_necessarias = ['PACIENTE', 'D.NASC', 'D.ATEND', 'GRAU', 'CONTATO', 'MUNICIPIO', 'MEDICO']
            for col in colunas_necessarias:
                if col not in df.columns:
                    flash(f"Coluna obrigatória ausente: {col}")
                    return redirect(url_for('importar_planilha'))

            df = df[colunas_necessarias].dropna(subset=['PACIENTE', 'D.NASC', 'GRAU'])
            ...
            df['PRIORIDADE'] = df['ESCORE'].apply(
                lambda x: 'Alta Prioridade' if x > 2 else ('Prioridade Intermediária' if x == 2 else 'Prioridade Eletiva')
            )
...
def confirmar_importacao():
    ...
    if 'temp_import_file' not in session:
        flash("Sessão de importação expirada. Por favor, envie a planilha novamente.", "error")
        return redirect(url_for('importar_planilha'))
    ...
    try:
        ...
        for dado in dados:
            ...
            """, (
                dado['nome'], 
                dado['nascimento'], 
                dado.get('contato', ''),  # Usa get() com valor padrão para evitar KeyError
                dado.get('municipio', ''),
    ...
    with get_db_connection() as conn:
        try:
            ...
            if nova_senha and nova_senha.standp():
                ...
                flash('Usuário atualizado com sucesso e nova senha definida como temporária.')
            else:
...
def editar_usuarios():
    ...
    with get_db_connection() as conn:
        usuarios = conn.execute("SELECT id, usuario, perfil FROM usuarios").fetchall()
...
def excluir_usuario(user_id):
    ...
    with get_db_connection() as conn:
        usuario = conn.execute("SELECT usuario FROM usuarios WHERE id = ?", (user_id,)).fetchone()
        ...
        admin_count = conn.execute("SELECT COUNT(*) FROM usuarios WHERE perfil = 'admin'").fetchone()[0]
        user_is_admin = conn.execute("SELECT COUNT(*) FROM usuarios WHERE id = ? AND perfil = 'admin'", (user_id,)).fetchone()[0]
...
def apagar_selecionados():
    ...
    try:
        with get_db_connection() as conn:
            ...
            conn.execute(f"DELETE FROM consultas WHERE paciente_id IN ({placeholders})", selecionados)
            
            # Depois apagar os pacientes
            conn.execute(f"DELETE FROM pacientes WHERE id IN ({placeholders})", selecionados)
            conn.commit()
        ...
        try:
            ...
            if acao == 'apagar_tudo':
                ...
                if selecionados:
                    ...
                    conn.execute(f"DELETE FROM operados WHERE id IN ({placeholders})", selecionados)
...
def nova_consulta(paciente_id):
    if session['perfil'] != 'medico' and session['perfil'] != 'admin':
        logger.warning(f"Unauthorized access attempt to nova_consulta by {session['usuario']}")
        flash("Acesso restrito a médicos e administradores.")
        return redirect(url_for('painel'))
    ...
    try:
        with get_db_connection() as conn:
            ...
            paciente = conn.execute("SELECT id, nascimento FROM pacientes WHERE id = ?", (paciente_id,)).fetchone()
            if not paciente:
            ...
            if request.method == 'POST':
                ...
                try:
                    ...
                    logger.info(f"Nova consulta registrada para paciente {paciente_id}")
                except Exception as e:
                    conn.execute("ROLLBACK")
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Invalid date format for patient {paciente_id}")
...
def agendar_cirurgia():
    ...
    try:
        with get_db_connection() as conn:
            ...
            paciente = conn.execute('SELECT id, nome FROM pacientes WHERE id = ?', (paciente_id,)).fetchone()
            if not paciente:
            ...
            try:
                ...
                data_formatada = datetime.strptime(data_cirurgia, '%Y-%m-%d').strftime('%d/%m/%Y')
            ...
            if agendamento_existente:
                ...
                logger.info(f"Agendamento atualizado para paciente {paciente_id}")
            else:
                ...
                logger.info(f"Novo agendamento criado para paciente {paciente_id}")
            
            conn.commit()
            
            # Registrar atividade no log
            logger.info(f"Cirurgia agendada com sucesso para o paciente (ID: {paciente_id}) para {data_formatada}")
...
def cancelar_realizacao(operado_id):
    ...
    try:
        with get_db_connection() as conn:
            ...
            cur = conn.cursor()
            cur.execute(
                'INSERT INTO pacientes (nome, nascimento, versao) VALUES (?, ?, 1)',  # Adicionado o campo versao
            ...
            # 2. Inserir consulta
            conn.execute(
                'INSERT INTO consultas (paciente_id, data, escore, prioridade, versao) VALUES (?, ?, ?, ?, 1)',  # Adicionado o campo versao
                (paciente_id, operado['data_consulta'], operado['escore'], operado['prioridade'])
            )
            ...
            # Registrar atividade no log
            logger.info(f"Realização de cirurgia cancelada para paciente {operado['nome']} (ID: {operado_id})")
            
            flash(f"Paciente {operado['nome']} retornou à lista de espera.", "success")
    ...
    try:
        with get_db_connection() as conn:
            ...
            if remover:
                ...
                logger.info(f"Tentando remover status judicial. Valores: prioridade_anterior={ultima_consulta['prioridade_anterior']}, escore_anterior={ultima_consulta['escore_anterior']}")
        ...
        logger.error(traceback.format_exc())  # Log adicional para rastrear o erro
...
def init_db():
    ...
    try:
        with get_db_connection() as conn:
            ...
            columns_pacientes = conn.execute("PRAGMA table_info(pacientes)").fetchall()
            column_names_pacientes = [col[1] for col in columns_pacientes]
            
            if 'versao' not in column_names_pacientes:
                conn.execute("ALTER TABLE pacientes ADD COLUMN versao INTEGER NOT NULL DEFAULT 1")
                
            # Verificar tabela consultas
            columns_consultas = conn.execute("PRAGMA table_info(consultas)").fetchall()
            column_names_consultas = [col[1] for col in columns_consultas]
            
            if 'versao' not in column_names_consultas:
                conn.execute("ALTER TABLE consultas ADD COLUMN versao INTEGER NOT NULL DEFAULT 1")
            
            # Outras verificações existentes...
            if 'escore_anterior' not in column_names_consultas:
                conn.execute("ALTER TABLE consultas ADD COLUMN escore_anterior INTEGER")
            
            if 'prioridade_anterior' not in column_names_consultas:
                conn.execute("ALTER TABLE consultas ADD COLUMN prioridade_anterior TEXT")
            
            if 'is_demanda_judicial' not in column_names_consultas:
                conn.execute("ALTER TABLE consultas ADD COLUMN is_demanda_judicial INTEGER DEFAULT 0")
            
            if 'data_judicial' not in column_names_consultas:
                conn.execute("ALTER TABLE consultas ADD COLUMN data_judicial TEXT")
                
            # Verificar se o usuário admin já existe
            admin_exists = conn.execute("SELECT COUNT(*) FROM usuarios WHERE usuario = 'admin'").fetchone()[0]
